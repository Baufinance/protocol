...
def test_velodrome_zap_deposit_withdraw_unsupported_token(zap, aggregationRouter, chain, common_health_check, base_fee_oracle,velodrome_factory, velodrome_router, gov, velo, registry, vault_template, treasury):
    token0 = gov.deploy(Token, 18)
    token1 = gov.deploy(Token, 18)
    ...
    zap.addFactory(velodrome_factory, {"from":gov})


    token1.approve(zap, 1_000*10**18, {"from":gov})

    data = aggregationRouter.encodeData([token1, token0, gov, zap, 500*10**18, 1000*10**18, 1])

    token0.mint(1_000*10**18, aggregationRouter)
    token1.mint(1_000*10**18, aggregationRouter)

    zap.zap(token1, pool, 500*10**18, data)
    ...
    token0_balance_before = token1.balanceOf(gov)

    vault_token_balance  = vault.balanceOf(gov)

    vault.approve(zap, vault_token_balance, {"from": gov})

    data = aggregationRouter.encodeData([token0, token1, gov, zap, vault_token_balance, 500*10**18, 1])

    zap.unzap(token1, pool, vault_token_balance, data)

    token0_balance_after = token1.balanceOf(gov)

    assert token0_balance_after - token0_balance_before



def test_velodrome_zap_deposit_withdraw_supported_token(zap, aggregationRouter, chain, common_health_check, base_fee_oracle,velodrome_factory, velodrome_router, gov, velo, registry, vault_template, treasury):
    token0 = gov.deploy(Token, 18)
    token1 = gov.deploy(Token, 18)
    ...
    token0.approve(zap, 1_000*10**18, {"from":gov})

    data = aggregationRouter.encodeData([token0, token1, gov, zap, 500*10**18, 1000*10**18, 1])

    token0.mint(1_000*10**18, aggregationRouter)
    token1.mint(1_000*10**18, aggregationRouter)

    zap.zap(token0, pool, 500*10**18, data)
    ...
    data = aggregationRouter.encodeData([token1, token0, gov, zap, vault_token_balance, 500*10**18, 1])

    zap.unzap(token0, pool, vault_token_balance, data)

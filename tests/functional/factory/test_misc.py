...
def test_create_2_coins_strategy_with_deposit_withdraw_in_target_coin_and_vault_deposit_fee_and_zap(zap, aggregationRouter, chain, rewards, treasury, common_health_check, base_fee_oracle,  cvx, crv, factory, curve_mock_builder, gov, weth, univ3_mock, univ2_mock, rewards_factory, booster, registry, vault_template):
  # try to build 2 pool with erc 20 tokens with plain and lending liquidity
  curve_mock_builder.build(2, False)
  ...
  data = aggregationRouter.encodeData([token2, token1, zap, zap, 500*10**18, 1000*10**18, 1])
  ...
  zap.unzap(token2, lp_token, vault_token_balance, data)

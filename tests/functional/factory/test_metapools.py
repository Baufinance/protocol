...
def test_create_metapool_3_metapool_strategy(chain, common_health_check, base_fee_oracle,  cvx, crv, factory, curve_mock_builder, gov, weth, univ3_mock, univ2_mock, rewards_factory, booster, registry, vault_template):
...
def test_create_3_metapool_strategy_with_deposit_withdraw_in_target_coin(chain, treasury, common_health_check, base_fee_oracle,  cvx, crv, factory, curve_mock_builder, gov, weth, univ3_mock, univ2_mock, rewards_factory, booster, registry, vault_template):
  curve_mock_builder.build(1, True)
  ...
  curve_pool = CurveMetaPoolMock.at(pool_address)
...
def test_create_3_metapool_strategy_with_deposit_withdraw_in_target_coin_and_vault_deposit_fee(chain, rewards, treasury, common_health_check, base_fee_oracle,  cvx, crv, factory, curve_mock_builder, gov, weth, univ3_mock, univ2_mock, rewards_factory, booster, registry, vault_template):
...
def test_create_3_metapool_strategy_with_deposit_withdraw_in_target_coin_and_vault_deposit_fee_and_zap_unzap_unsupported_coin(zap, aggregationRouter, chain, rewards, treasury, common_health_check, base_fee_oracle,  cvx, crv, factory, curve_mock_builder, gov, weth, univ3_mock, univ2_mock, rewards_factory, booster, registry, vault_template):
  ...
  token1 = Token.at(pool.coins(0))
  pool_2 = Curve3PoolMock.at(pool.coins(1))

  token2 = Token.at(pool_2.coins(0))


  token1.mint(10_000*10**18, aggregationRouter, {"from": gov})
  token2.mint(10_000*10**18, aggregationRouter, {"from": gov})

  curve_zap = gov.deploy(CurveZapMetaPoolMock)

  curve_zap.addPool(pool, pool, {"from":gov})

  token1.mint(3000*10**18, {"from":gov})

  token1.approve(curve_zap, 3000*10**18, {"from":gov})
  ...
  factory.setZapContract(lp_token, curve_zap)
...
def test_create_3_metapool_strategy_with_deposit_withdraw_in_target_coin_and_vault_deposit_fee_and_zap_unzap_targetcoin(zap, aggregationRouter, chain, rewards, treasury, common_health_check, base_fee_oracle,  cvx, crv, factory, curve_mock_builder, gov, weth, univ3_mock, univ2_mock, rewards_factory, booster, registry, vault_template):
  curve_mock_builder.build(1, False)
...
def test_create_3_metapool_strategy_with_deposit_withdraw_in_target_coin_and_change_target_coin(chain, treasury, common_health_check, base_fee_oracle,  cvx, crv, factory, curve_mock_builder, gov, weth, univ3_mock, univ2_mock, rewards_factory, booster, registry, vault_template):
  # try to build 2 pool with erc 20 tokens with plain and lending liquidity
  ...
  token2.mint(10_000*10**18, univ3_mock, {"from": gov})
  ...
  token2.mint(10_000*10**18, curve_zap, {"from": gov})

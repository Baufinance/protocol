...
def test_setters(
    ...
    if is_convex:
        strategy.setKeep(10, 0, gov, voter, {"from": gov})
        strategy.setClaimRewards(True, {"from": gov})
        strategy.setHarvestTriggerParams(90000e6, 150000e6, 1e24, False, {"from": gov})
    else:
        strategy.setKeepCRV(0, voter, {"from": gov})
    try:
    ...
    if is_convex:
        with brownie.reverts():
            strategy.setKeep(10_001, 0, gov, voter, {"from": gov})
        with brownie.reverts():
            strategy.setKeep(0, 10_001, gov, voter, {"from": gov})
    else:
        with brownie.reverts():
            strategy.setKeepCRV(10_001, voter,{"from": gov})
